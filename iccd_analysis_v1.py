...
filePath = './iccd/hiper-pit'
fileName = 'shot 2484.spe' # '2021 October 04 17_24_04.spe'
fullPath = join(filePath, fileName)
...
logPath = '/home/curtis/Documents/research/uw/spreadsheets'
...
shotNums = shotList[lastIndices]  # [2480, 2481, 2482, 2484]
iccdDelay = iccdDelay[lastIndices]
filenames = [join(filePath, 'shot ' + str(int(shotNum)) + '.tif') for shotNum in shotNums]
images = []
...
imageio.imwrite('./iccd/test.tif', images[-1])
data = imageio.imread('./iccd/test.tif')
plt.figure()
...
fig1, ax1 = plt.subplots(4, 6, sharex=True, sharey=True, figsize=(16, 10), gridspec_kw=dict(wspace=0.0, hspace=-0.1))
fig2, ax2 = plt.subplots(4, 6, sharex=True, sharey=True, figsize=(16, 10), gridspec_kw=dict(wspace=0.0, hspace=0.0))
...
for count, image in enumerate(images):
    ...
    ax2[i, j].text(5, 25000, str(np.round(iccdDelay[count] - 17.05, 2)) + '$\mu$s', fontsize=12, color='black')
    if j == 0:
...
ax2[0, 0].legend(['Along X', 'Along Y'], ncol=2, bbox_to_anchor=(0, 1.01, 1, 0.1), loc="lower left",
